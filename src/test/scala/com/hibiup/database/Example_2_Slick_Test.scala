...
class Example_2_Slick_Test extends Init{
    ...
    "Slick" should "" in {
        ...
        withResource(Database.forConfig("database.connection")){ conn =>
            /**
              * 4) 指令模版，这很重要，下面的指令，包括 filter, drop, sortBy 等都是基于这个模版来生成最终的指令
              * */
            ...
            val query1 = search.result.map(_.headOption.map(u => User(u.id, u.email, Option(null), u.lasttime)))
            ...
            val query2 = search.map(u => (u.id, u.email, u.lasttime))
                ...
                case (userId, email, lastLoginTime) => User(userId, email, Option(null), lastLoginTime)
            })
